...
namespace ProgrammingAssignment4
{
    ...
    public class TeddyBear
    {
        ...
        public TeddyBear(Texture2D sprite, Vector2 location)
        {
            ...
            //set X and Y of drawRectange as X and Y of location
            drawRectangle.X = (int)location.X;
            drawRectangle.Y = (int)location.Y;

            //set width and height of drawRectangle as width and height of sprite
        ...
        public void Update(GameTime gameTime, MouseState mouse)
        {
            ...
            if(collecting)
            {
                //update location field using gameTime and velocity
        ...
        public void SetTarget(Vector2 target)
        {
            ...
            //Set the X/Y of velocity to the difference between target X/Y and teddy bear X/Y
            velocity.X = target.X - location.X;
            velocity.Y = target.Y - location.Y;

            //Convert the velocity to a unit vector using Normalize method
            velocity.Normalize();

            //multiply the velocity by BaseSpeed; *= means variable is equal to the value of the variable times BaseSpeed
            velocity *= BaseSpeed;
        }
